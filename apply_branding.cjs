...
// 1. UPDATE BROWSER TAB TITLE TO "RAK's"
const indexPath = path.join(process.cwd(), 'index.html');
if (fs.existsSync(indexPath)) {
  let indexCode = fs.readFileSync(indexPath, 'utf8');
  indexCode = indexCode.replace(/<title>.*<\/title>/, "<title>RAK's Workspace</title>");
  fs.writeFileSync(indexPath, indexCode);
}

// 2. REWRITE AUTH.TSX WITH FORGOT PASSWORD & NEW BRANDING
...
console.log("✅ Rebranded to RAK's and added Forgot Password functionality!");
