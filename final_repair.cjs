...
export default function TaskList({ activeView, activeListId, activeTagName, activeTaskId, onSelectTask }) {
  ...
  const [sortByPriority, setSortByPriority] = useState(false); // NEW: Sorting State
  
  // 1. Safely fetch ALL tasks once
  const allTasks = useLiveQuery(() => db.tasks.toArray(), []) || [];
  
  // 2. Perform robust filtering and sorting in-memory
  const displayTasks = useMemo(() => {
    ...
    if (activeView === 'today') {
      ...
      filtered = filtered.filter(t => !t.listId); // Inbox
    }

    // Sort logic
    return filtered.sort((a, b) => {
      // If priority sort is ON, sort by Priority first (High to Low)
      if (sortByPriority && b.priority !== a.priority) {
        return b.priority - a.priority;
      }
      // Otherwise (or if priorities are tied), sort by Newest first
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      <div className="space-y-1">
        ...
        {displayTasks?.length === 0 && <div className="text-center text-gray-400 py-10">No tasks for this view. Enjoy your day!</div>}
      </div>
    </div>
  );
}`;
...
console.log("✅ Repair Complete! Drag & Drop, Smart Lists, and Priority Sorting are perfectly active.");
