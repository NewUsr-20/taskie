...
export default function GlobalSearch({ isOpen, onClose, onNavigate }) {
  ...
  // CLOUD CONNECTION: Pulling search data directly from Supabase Zustand store
  const { tasks: allTasks, notes: allNotes, lists: allLists, tags: allTags } = useTaskStore();
    ...
    <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[10vh] bg-gray-900/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        <div className="flex items-center px-4 py-3 border-b border-gray-100">
          ...
          <input autoFocus type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search tasks, notes, folders, and tags..." className="flex-1 text-lg bg-transparent outline-none text-gray-800 placeholder-gray-400" />
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2">
          {!query.trim() ? (
            ...
            <div className="space-y-1">
              {results.map((res, i) => (
                <div key={i} onClick={() => handleSelect(res)} className="flex items-center gap-4 p-3 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
