...
import { Inbox, Hash, ListTodo, Plus, Trash2 } from 'lucide-react';
...
function DroppableSidebarItem({ id, type, name, activeId, onClick, children, onDelete }) {
  ...
  return (
    ...
    >
      ...
      {onDelete && !isOver && (
        <button onClick={onDelete} className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 transition-opacity">
...
export default function Sidebar({ activeView, setActiveView, activeListId, setActiveListId, activeTagName, setActiveTagName, isOpen, setIsOpen }) {
  ...
  return (
    <aside className="w-64 bg-[#f8f9fb] border-r border-gray-200 flex flex-col h-full overflow-hidden transition-all">
      <div className="h-16 flex items-center px-6 border-b border-gray-200">
        <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white font-bold shadow-md">T</div>
        <span className="ml-3 font-bold text-gray-800 tracking-tight">TickTick Clone</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-8">
        <div className="space-y-1">
          <button 
            ...
            className={\`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all \${activeView === 'list' && !activeListId ? 'bg-blue-100 text-blue-700 shadow-sm font-semibold' : 'text-gray-600 hover:bg-gray-100'}\`}
          >
        ...
        <div>
          <div className="flex items-center justify-between px-3 mb-3 text-xs font-bold text-gray-400 uppercase tracking-widest">
            Lists
          </div>
          <div className="space-y-1">
...
console.log("🎨 UI Enhanced! Drop zones will now highlight when dragging.");
